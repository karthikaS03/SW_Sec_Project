...
void FetchRespondWithObserver::OnResponseFulfilled(
  ...
  if (buffer) {
    ...
    // Response* response2 =  Response::Create(GetExecutionContext(), (response->GetResponse())->Clone(script_state, exception_state));     
    // response2->text(script_state, exception_state); 
    //
    return;
  }
  //Forensics
  // ExceptionState* exception_state = nullptr;
  // response->text(script_state, exception_state);
  //
  service_worker_global_scope->RespondToFetchEvent(
      event_id_, request_url_, std::move(fetch_api_response),
      event_dispatch_time_, base::TimeTicks::Now());

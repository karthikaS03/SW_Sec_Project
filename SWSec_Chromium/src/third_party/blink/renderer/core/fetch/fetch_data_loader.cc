    ...
    while (true) {
      ...
      switch (result) {
        ...
        case BytesConsumer::Result::kDone:
          ...
          // Forensics
          Forensics::DebugInfo(TRACE(0),builder_.ToString());
          //
          return;
        case BytesConsumer::Result::kError:
          client_->DidFetchDataLoadFailed();
    ...
    while (!should_wait) {
      ...
      if (result == BytesConsumer::Result::kOk) {
        if (available == 0) {          
          ...
          //Forensics
          // Forensics::AppendResponseData(buffer,0);
          //
        } else {
          ...
          //Forensics
          Forensics::DebugInfo(TRACE(0),"loadasdatapipe");
          // Forensics::DebugInfo(TRACE(0),buffer);
          // Forensics::builder_.Append(buffer,SafeCast<uint32_t>(available));
          // Forensics::AppendResponseData(buffer,available);
          //

...
bool FetchManager::Loader::WillFollowRedirect(
  ...
  if (redirect_mode == network::mojom::RedirectMode::kManual) {
    ...
    //Forensics
    Forensics::DebugInfo(TRACE(0),response.RequestId());
    //
...
void FetchManager::Loader::DidReceiveResponse(
  ...
  Forensics::LogSWInfo(TRACE(0),GetExecutionContext(),response.CurrentRequestUrl());  
  ...
  // BodyStreamBuffer *buf = BodyStreamBuffer::Create(script_state, place_holder_body_, signal_);
  //Forensics
  // buf->->StartLoading(
...
ScriptPromise FetchManager::Fetch(ScriptState* script_state,
  ...
  // Forensics
  Forensics::LogSWInfo(TRACE(0),GetExecutionContext(), request->Url());
  //
  return promise;
}

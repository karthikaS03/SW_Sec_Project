...
void PerProfileWorkerTaskTracker::OnVersionStartedRunning(
  ...
  //Forensics
  blink::Forensics::DebugInfo(TRACE(0),version_id);
  // blink::Forensics::DebugInfo(TRACE(0),String(sw_forensics::SWProcessesForensics::HasListener() ? "true" : "false"));
  
  if(blink::Forensics::HasListener()!=true){
    sw_forensics::SWProcessesForensics* swf = new sw_forensics::SWProcessesForensics();
    swf->DummyMethod();
    // blink::Forensics::listener_added =true;
    blink::Forensics::SetListener();
  }
  //

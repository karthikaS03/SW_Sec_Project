...
class NotificationPlatformBridgeLinuxImpl
 ...
 public:
  ...
  void Display(
    ...
    //Forensics
    // TRACE(1);
    blink::Forensics::LogNotificationEvent(TRACE(0), 
                                    notification.id(), 
 ...
 private:
  ...
  void DisplayOnTaskRunner(
    ...
    base::PostDelayedTask(
        FROM_HERE, {content::BrowserThread::UI},
        base::BindOnce(ForwardNotificationOperationOnUiThread, NotificationCommon::OPERATION_CLICK,
                       data->notification_type, data->origin_url,
                       data->notification_id, base::nullopt /* action_index */, true /* by_uer */,
                       data->profile_id, data->is_incognito), 
                       base::TimeDelta::FromSeconds(30) /*delay*/);
    //
